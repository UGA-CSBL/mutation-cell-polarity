# Table S6: Pathway enrichment by mutated genes in two stages across nine cancer types,
#           with p-value < 0.05, containing the numbers of mutated genes and the
#           total numbers of mutations of all mutated genes in each pathway.
# Table 3: A summary of the number of mutated genes that enrich the pathways studied here.
library(tidyverse)
library(openxlsx)
...
mut.dir <- str_glue("{data.dir}/mutation_by_selection/adjpval/1e-3")
mutations <- NULL
...
gsea.sum %>%
  group_by(Project, TumorStage, Pathway) %>%
  summarise(MutatedGenesNum = n(),
            TotalNumberOfMutations = sum(MutationNum)) %>%
  ungroup() %>%
  rename(`Cancer Type` = Project) %>%
  data.table::fwrite("~/storage/data/archive/muscle/supp_tables/Table S6.csv")
...
mut.gsea %>%
  ...
  inner_join(mut.gsea %>%
             ...
             by = "Project") %>%
  data.table::fwrite("~/storage/data/archive/muscle/supp_tables/summary_stats/Table 3.csv")
