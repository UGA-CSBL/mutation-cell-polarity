...
#       pivot_wider(names_from = "Stage", values_from = "Regulation") %>%
#       mutate(Early = map_chr(Early, ~ paste(.x, collapse = ", ")),
#              Late = map_chr(Late, ~ paste(.x, collapse = ", "))) %>%
#       filter(Early == "Up" & Late == "Up") %>%
#       mutate(Regulation = "x") %>%
#       select(Project, Pathway, Regulation),
#     by = "Pathway") %>%
#   pivot_wider(names_from = "Project", values_from = "Regulation") %>%
#   select(Pathway, one_of(str_remove(projects, "^TCGA-"))) %>%
#   openxlsx::write.xlsx("~/storage/data/archive/muscle/supp_tables/78_pathways_unique_up_in_early_and_late.xlsx")
...
pathways %>%
  ...
  left_join(
    dea.gsea %>%
      ...
      mutate(Early = map_chr(Early, ~ paste(.x, collapse = ", ")),
             Late = map_chr(Late, ~ paste(.x, collapse = ", "))) %>%
      filter(str_detect(Early, "Up") & str_detect(Late, "Up")) %>%
      mutate(Regulation = "x") %>%
      select(Project, Pathway, Regulation),
    by = "Pathway") %>%
  pivot_wider(names_from = "Project", values_from = "Regulation") %>%
  select(Pathway, one_of(str_remove(projects, "^TCGA-"))) %>%
  openxlsx::write.xlsx("~/storage/data/archive/muscle/supp_tables/78_pathways_seen_up_in_early_and_late.xlsx")
...
pathways %>%
  ...
  openxlsx::write.xlsx("~/storage/data/archive/muscle/supp_tables/78_pathways_seen_up_in_early_or_late.xlsx")
