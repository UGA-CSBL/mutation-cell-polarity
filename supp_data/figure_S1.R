...
ggsave(str_glue("~/storage/data/archive/muscle/supp_tables/Figure S1.tiff"),
       p, width = 12, height = 13, units = "in", dpi = "print")

# Top 10 genes -----
mut.filtered <- mut.summary %>%
  group_by(Project) %>%
  top_n(10, NormMutations) %>%
  ungroup()

dat <- dat.tpm %>%
  filter(Symbol %in% mut.filtered$Symbol) %>%
  distinct(Project, Symbol, Barcode, .keep_all = T) %>%
  inner_join(select(mut.filtered, -TumorStage), by = c("Project", "Symbol")) %>%
  arrange(NormMutations) %>%
  mutate(Symbol = str_glue("{Symbol} ({round(NormMutations, 4)})")) %>%
  # Tweak for plotting
  mutate(TumorStage = factor(TumorStage),
         Project = str_remove(Project, "^TCGA-"),
         TPM = log(TPM + 0.1))

p <- ggboxplot(dat, x = "Symbol", y = "TPM",
               group = "TumorStage", color = "TumorStage") %>%
  facet(facet.by = "Project", nrow = 3, scales = "free") %>%
  ggpar(xlab = "Normalized # of mutations",
        ylab = "Average expression in ln(TPM + 0.1)",
...
ggsave(str_glue("~/storage/data/archive/muscle/supp_tables/mutation_vs_avg_expression_top10.png"),
       p, width = 14, height = 13, units = "in", dpi = "print")

# Density of normalized # of mutations -----
# mut.summary %>%
#   mutate(NormMutations = log(NormMutations)) %>%
#   gghistogram(x = "NormMutations", color = "TumorStage", bins = 30) %>%
#   facet("Project", nrow = 3, scales = "free") %>%
#   ggpar(xlab = "Normalized # of mutations",
#         legend.title = "",
#         font.main = c(18, "bold", "black"),
#         font.legend = 16, font.x = 16, font.y = 16,
#         palette = "jco")
#
# ggsave("~/storage/data/archive/muscle/supp_tables/log_normalized_mutations.png",
#        width = 12, height = 13, units = "in", dpi = "print")
