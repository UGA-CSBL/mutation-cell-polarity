...
if (!file.exists(str_glue("~/storage/data/archive/muscle/mutation_freq/mutations_gene_length_{length.cutoff}.xlsx"))) {
  openxlsx::write.xlsx(list(Mutations = mutations, Summary = mutation.summary),
                       file = str_glue("~/storage/data/archive/muscle/mutation_freq/mutations_gene_length_{length.cutoff}.xlsx"))
}
...
if (!dir.exists(str_glue(
  "~/storage/data/archive/muscle/mutation_freq/GSEA_summary_gene_length_{length.cutoff}"))) {
  dir.create(str_glue(
    "~/storage/data/archive/muscle/mutation_freq/GSEA_summary_gene_length_{length.cutoff}"))
}
...
registerDoSNOW(cl)
foreach (proj = projects,.export = "length.cutoff",
         .packages = c("tibble", "tidyr", "purrr", "dplyr", "stringr", "reactable", "openxlsx")) %dopar%
  {
    ...
    for (stage in stages) {
      ...
      gsea.res <- openxlsx::read.xlsx(str_glue(
        "~/storage/data/archive/muscle/mutation_freq/GSEA_gene_length_{length.cutoff}/{proj}.xlsx"
      ), sheet = stage) %>%
        as_tibble() %>%
        filter(pval <= 0.2)
      ...
      reactable(pw.data, searchable = T, defaultPageSize = 20,
        ...
        htmltools::save_html(file = str_glue("~/storage/data/archive/muscle/mutation_freq/GSEA_summary_gene_length_{length.cutoff}/{proj}_{stage}.html"))
    }
    openxlsx::write.xlsx(
      proj.res,
      file = str_glue("~/storage/data/archive/muscle/mutation_freq/GSEA_summary_gene_length_{length.cutoff}/{proj}.xlsx")
    )
  }
stopCluster(cl)
...
# [10] purrr_0.3.4       readr_1.3.1       tidyr_1.0.3       tibble_3.0.1      ggplot2_3.3.0     tidyverse_1.3.0
#
# loaded via a namespace (and not attached):
# [1] tidyselect_1.0.0    haven_2.2.0         lattice_0.20-41     colorspace_1.4-1    vctrs_0.2.4         generics_0.0.2      htmltools_0.4.0     rlang_0.4.6
# [9] pillar_1.4.4        glue_1.4.0          withr_2.2.0         DBI_1.1.0           BiocParallel_1.22.0 dbplyr_1.4.3        modelr_0.1.7        readxl_1.3.1
# [17] lifecycle_0.2.0     munsell_0.5.0       gtable_0.3.0        cellranger_1.1.0    zip_2.0.4           rvest_0.3.5         htmlwidgets_1.5.1   codetools_0.2-16
# [25] parallel_4.0.0      fansi_0.4.1         broom_0.5.6         Rcpp_1.0.4.6        scales_1.1.0        backports_1.1.6     jsonlite_1.6.1      fs_1.4.1
# [33] gridExtra_2.3       fastmatch_1.1-0     digest_0.6.25       hms_0.5.3           openxlsx_4.1.5      stringi_1.4.6       grid_4.0.0          cli_2.0.2
# [41] tools_4.0.0         magrittr_1.5        crayon_1.3.4        pkgconfig_2.0.3     ellipsis_0.3.0      Matrix_1.2-18       data.table_1.12.8   xml2_1.3.2
